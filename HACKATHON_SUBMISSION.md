...
`GcpHsmAdapter` (in `services/crypto-core/`) integrates Cloud KMS directly, and
the integration was verified against a real HSM-protected `secp256k1` key in
`northamerica-northeast1`. We are not pursuing other bounties.

> Note on the algorithm: Google Cloud HSM does not offer Ed25519 at HSM
> protection level, so the HSM-backed share uses **secp256k1** (also Ethereum's
> curve). This choice is scoped to the demonstration; Proof of Learning's
> production signature scheme is governed by its own decision records.
